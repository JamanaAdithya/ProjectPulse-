...
const calculateDuration = require("../helpers/calculateDuration");
// Get all time logs
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch time log", details: error.message });
  }
});
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!user || !startTime || !project) {
      return res
        .status(400)
        .json({ error: "user, start time, project are required" });
    }

    const computedDuration = endTime
    ...
    if (task) {
      await Task.findByIdAndUpdate(newTimeLog.task, {
        $push: { timeLogs: newTimeLog._id },
      });
    }
    res
      .status(201)
      .json({ message: "TimeLog created successfully", timeLog: newTimeLog });
  } catch (error) {
    res
      .status(500)
      .json({ error: "Failed to create time log", details: error.message });
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (!user || !project || !startTime) {
      return res
        .status(400)
        .json({ error: "User, project, and startTime are required" });
    }
    const updatedTimeLog = await TimeLog.findByIdAndUpdate(
      req.params.id,
      { user, project, task, startTime, endTime, duration: computedDuration },
    ...
    res
      .status(500)
      .json({ error: "Failed to update time log", details: error.message });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete time log", details: error.message });
  }
});
